...
exports.backToPublishHandler = backToPublishHandler;
const grammy_1 = require("grammy");
async function backToPublishHandler(ctx) {
    if (!ctx.callbackQuery?.data)
        return;
    await ctx.answerCallbackQuery();
    const data = ctx.callbackQuery.data;
    const productId = data.split("_")[3];
    const keyboard = new grammy_1.InlineKeyboard().text("🟢 انتشار محصول", `select_time_${productId}`);
