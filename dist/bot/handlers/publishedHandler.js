...
exports.publishedHandler = publishedHandler;
const grammy_1 = require("grammy");
const wordpressService_1 = require("../../services/wordpressService");
async function publishedHandler(ctx) {
    ...
    try {
        const response = await (0, wordpressService_1.getAllProducts)(page);
        const { data: products, page: current_page, total_pages } = response;
        if (!products || products.length === 0)
            return await ctx.reply("❌ هیچ محصولی برای نمایش وجود ندارد.");
        ...
        if (total_pages > 1) {
            if (current_page > 1)
                keyboard.text("➡️ صفحه قبل", `published_page_${current_page - 1}`);
            if (current_page < total_pages)
                keyboard.text("صفحه بعد ⬅️", `published_page_${current_page + 1}`);
            keyboard.row();
        }
